...
class ImageMover {
  ...
  ImageMover(String nomVig, int w, int h) {
    ...
    // println(dateTxt);
    // Chargement de l'image (vignette ou image suivant la taille)
    // println("size.x="+size.x+", size.y="+size.y+", tW="+tW+", tH="+tH);
    if (size.x > tW) {
      img = loadImage(nomImage);
      // println("Chargement image:"+nomImage);
    } else {
      img = loadImage(nomThumb);
      // println("Chargement vignette:"+nomThumb);
    }
  ...
  void display() {
    if (img==null) {
      ...
      fill(255);
      textSize(9);
      text(dateTxt, pos.x, size.y+pos.y-9, size.x); 
    }
  }
